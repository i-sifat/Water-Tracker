...
void main() {
  ...
  group('DeviceService', () {
    ...
    group('getDeviceId', () {
      ...
      test(
        'should return the same device ID on subsequent calls (caching)',
        () async {
    ...
    group('generateUniqueCode', () {
      ...
      test(
        'should handle errors gracefully with fallback code generation',
        () async {
          ...
          expect(
            code,
            matches(
              RegExp(r'^[A-F0-9]{3}-[A-F0-9]{3}-[A-F0-9]{3}-[A-F0-9]{3}$'),
            ),
          );
        },
      );
