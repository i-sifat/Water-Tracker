...
class HydrationProvider extends ChangeNotifier {
  ...
  Future<void> _loadData() async {
    try {
      ...
      final avatarString =
          await _storageService.getString('avatar', encrypted: false)
              as String?;
      _selectedAvatar =
          avatarString == 'female' ? AvatarOption.female : AvatarOption.male;

      final lastUpdatedMillis =
          await _storageService.getInt('lastUpdated') as int?;
      _lastUpdated =
          lastUpdatedMillis != null
              ? DateTime.fromMillisecondsSinceEpoch(lastUpdatedMillis)
              : null;

      final lastGoalAchievedMillis =
          await _storageService.getInt('lastGoalAchievedDate') as int?;
  ...
  Future<void> _loadHistoricalData() async {
    try {
      final historyJson =
          await _storageService.getString('hydrationHistory') as String?;
      if (historyJson != null) {
  ...
  Future<void> loadCustomDrinkTypes() async {
    try {
      final customDrinkTypesJson =
          await _storageService.getString('customDrinkTypes') as String?;
