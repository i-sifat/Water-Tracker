...
class PremiumProvider extends ChangeNotifier {
  ...
  Future<void> _loadOrGenerateDeviceCode() async {
    try {
      ...
      var deviceCode = (deviceCodeData is String) ? deviceCodeData : null;
  ...
  /// Generate device code (alias for regenerateDeviceCode)
  Future<String> generateDeviceCode() async {
    await regenerateDeviceCode();
    return _premiumStatus.deviceCode;
  }

  /// Load premium status (alias for _loadPremiumStatus)
  Future<void> loadPremiumStatus() async {
    await _loadPremiumStatus();
  }

  /// Save premium status (alias for _savePremiumStatus)
  Future<void> savePremiumStatus() async {
    await _savePremiumStatus();
  }

  /// Check if has advanced analytics
  bool get hasAdvancedAnalytics =>
      isFeatureUnlocked(PremiumFeature.advancedAnalytics);

  /// Check if has custom reminders
  bool get hasCustomReminders =>
      isFeatureUnlocked(PremiumFeature.customReminders);
