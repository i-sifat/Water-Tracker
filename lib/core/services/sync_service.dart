...
class SyncService {
  ...
  Future<String?> createLocalBackup({String? customName}) async {
    ...
    try {
      ...
      backupData['backup_metadata'] = {
        ...
        'data_types':
            backupData.keys.where((k) => k != 'backup_metadata').toList(),
      };
  ...
  Future<bool> restoreFromLocalBackup(String backupPath) async {
    ...
    try {
      ...
      await createLocalBackup(
        customName: 'pre_restore_${DateTime.now().millisecondsSinceEpoch}',
      );
  ...
  Future<SyncResult> syncWithCloud() async {
    ...
    try {
      ...
      final syncResponse = await _performCloudSync(
        deviceId,
        localChanges,
        lastSyncTimestamp,
      );
  ...
  Future<void> performAutoSync() async {
    ...
    if (!(settings['autoSyncEnabled'] as bool? ?? false) ||
        !await isCloudSyncAvailable()) {
      return;
    }
  ...
  Future<Map<String, dynamic>> _collectAllData() async {
    ...
    return {
      'hydration_data':
          await _storageService.getJson('hydration_history') ?? {},
      'user_profile': await _storageService.getJson('user_profile') ?? {},
      'app_preferences': await _storageService.getJson('app_preferences') ?? {},
      'notification_settings':
          await _storageService.getJson('notification_settings') ?? {},
      'premium_status':
          await _storageService.getBool('premium_status') ?? false,
    };
  }

  Future<Map<String, dynamic>> _collectSelectedData(
  ...
  Future<List<Map<String, dynamic>>> _getLocalChangesSince(
    ...
    return syncQueue
        .where((item) => (item['timestamp'] as int) > timestamp)
        .toList();
  }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CloudSyncResponse(
          ...
          remoteChanges:
              (responseData['remote_changes'] as List<dynamic>)
      ...
      return CloudSyncResponse(success: false, error: 'Network error: $e');
    }
  }
